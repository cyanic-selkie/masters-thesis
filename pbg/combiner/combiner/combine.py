...
def main():
	...
	embeddings = np.memmap(args.output, dtype=np.float32, mode="w+", shape=(args.nodes, args.dimensions))
	idx = 0
	for i in range(args.partitions):
		with h5py.File(join(args.embeddings_dir, f"embeddings_all_{i}.v40.h5"), "r") as f:
