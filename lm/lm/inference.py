...
def get_candidates(index, lemmatizer, query):
    ...
    query = [
        token.lemma_.lower() for token in lemmatizer(query)
...
def disambiguate(text: str, mentions: List[Tuple[int, int]], top_k: int,
    ...
    padded_document_token_spans = [
        spans + [(0, 0)] * (max_length - len(spans))
    ...
    order = []
    for i, (token_spans, predicted_embeddings, input_ids) in enumerate(
            zip(document_token_spans, document_predicted_embeddings,
                inputs["input_ids"])):
        for j, ((x, y), predicted_embedding) in enumerate(
                zip(token_spans, predicted_embeddings)):
            if (x == 0 and y == 0):
                break
            ...
            prediction = [(candidate_qids[k], preferred_names[k],
                           fts_scores[k], scores[k])
                          for k in reversed(top_k_indices)]
...
if __name__ == "__main__":
    ...
    mentions: List[Tuple[int,
                         int]] = [(45, 53), (155, 178), (181, 184), (329, 332),
                                  (500, 506), (705, 711), (1031, 1039),
                                  (1605, 1612), (1615, 1621), (1630, 1644),
                                  (1708, 1714), (1840, 1846), (1998, 2007),
                                  (2256, 2265), (2396, 2404)]
    qids: List[int] = [
        1781, 851259, 851259, 851259, 1741, 142, 1781, 28, 36, 213, 142, 142,
        28, 171357, 1781
    ]
