...
if __name__ == "__main__":
    ...
    for mention in mentions:
        ...
        spans.append(get_span_index(start, end, inputs["input_ids"].shape[1]))
    ...
    for span in spans:
        ...
        candidates, scores, names = get_candidates(index, lemmatizer, mention, nodes)
        ...
        if len(candidates) > 0:
            ...
            qids = [(candidates[i], names[i], scores[i]) for i in indices]

        qids.sort(key=lambda x: x[2])
